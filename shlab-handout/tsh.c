...
void eval(char *cmdline)
{
    char *argv[MAXARGS]; // 用于存储命令行参数的数组
    int bg;              // 标志，表示命令是否在后台运行
    pid_t pid;           // 进程ID
    sigset_t mask;       // 用于阻塞和解除阻塞信号的信号集合

    // 解析命令行参数，填充 argv 数组，并返回是否在后台运行的标志
    bg = parseline(cmdline, argv);

    // 检查命令是否为内置命令，如果是，则执行内置命令并返回
    if (!builtin_cmd(argv))
    {

        // 阻塞 SIGCHLD 信号，防止在添加作业前子进程就绪导致 race condition
        sigemptyset(&mask);
